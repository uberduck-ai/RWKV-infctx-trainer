...
class RWKV(L.LightningModule):
    ...
    def compute_loss(self, batch, batch_idx, is_training_run: bool):
        if (self.random_chop_prob == 1.0) or (self.random_chop_prob > 0.0 and random() < self.random_chop_prob):
            if len(batch['input_ids'][0]) > self.ctx_len:
                random_chop_start = randint(0, len(batch['input_ids'][0]) - self.ctx_len)
                for j in ('input_ids', 'attention_mask'): # [NOTE] token_type_ids is unused?
                    batch[j] = batch[j][:, random_chop_start:(random_chop_start + self.ctx_len)]
        seq = batch['input_ids']
